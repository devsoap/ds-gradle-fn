      ...
      steps {
        withCredentials([[$class: 'UsernamePasswordMultiBinding', credentialsId: 'GithubID', usernameVariable: 'GIT_USERNAME', passwordVariable: 'GIT_PASSWORD']]) {
          ...
          dir('/tmp/docs') {
            ...
            sh "git checkout -b fn-gradle-plugin/${params.buildVersion}"
            sh "git add docs/_fn_gradle_plugin_api"
            sh "git commit -m 'Update API documentation for FN Gradle Plugin ${params.buildVersion}'"
            sh "git remote add docs https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/devsoap/docs.git"
            sh "git push docs fn-gradle-plugin/${params.buildVersion}"
          }
        }
      }
    }
